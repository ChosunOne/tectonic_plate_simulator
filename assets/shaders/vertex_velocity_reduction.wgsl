@group(0) @binding(0) var<storage, read> vertex_velocity: array<vec2<f32>>;
@group(0) @binding(1) var<storage, read_write> velocity_bounds: array<f32>;
...
fn main(@builtin(local_invocation_id) local_id: vec3<u32>) {
    ...
    for (var i = lid; i < num_vertices; i   += 64u) {
        let mag = vertex_velocity[i].x;
        thread_min = min(thread_min, mag);
        thread_max = max(thread_max, mag);
    }
    ...
    if lid == 0u {
        velocity_bounds[0] = local_min[0];
        velocity_bounds[1] = local_max[0];
    }
}
