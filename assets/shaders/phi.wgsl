...
@group(2) @binding(4) var<storage, read> vertex_edge_data: array<u32>;
...
fn get_dividing_edge(cell_a: u32, cell_b: u32) -> u32 {
    for (var i = 0u; i < 3u; i++) {
        for (var j = 0u; j < 3u; j++) {
            if cell_edge_data[cell_a * 3u + i] == cell_edge_data[cell_b * 3u + j] {
                return cell_edge_data[cell_a * 3u + i];
            }
        }
    }
...
fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    for (var i: u32 = 0u; i < 3u; i++) {
        let neighbor_idx = cell_cell_data[cell_idx * 3u + i];
